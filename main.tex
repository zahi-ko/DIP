...
计算得到 Weber 对比度后，利用 \texttt{summary} 函数提取描述性统计量（最小值、四分位数、中位数、最大值等），为后续增强算法的参数优化提供定量依据。结果如下：
% TODO 在这里插入表格显示结果
...
本课程设计采用 MATLAB 的 \texttt{histogram} 函数构建直方图，结果如图\ref{histo_w}所示。
% TODO 分析直方图
...
\begin{enumerate}
    ...
    \item 选用感知均匀色图（实验中采取的为 \texttt{pink}）以保证色彩变化的单调性与视觉可辨性；
    \item 附加色标（Colorbar）建立数值-色彩对应关系的图例，实现定量分析。
\end{enumerate}

热力图可视化保留了 Weber 对比度的空间拓扑结构，能够直观揭示图像中边缘、纹理等高频成分的空间分布特征。高对比度区域（$|C_W| \gg 0$）对应图像的显著性特征，而低对比度区域（$|C_W| \approx 0$）则表征平坦的背景区域。
