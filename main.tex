...
与\ref{cha:3}章的两种重建增强（伽马变换与全局直方图均衡化）相比，式\ref{eq:enh_transform} 的组合模型在保留细节、控制噪声与维持整体亮度方面更为平衡，同时，在图\ref{fig:w_histoeq}中出现的块状失真也得到了显著改善。本方法以局部对比度为导向、以背景照度为锚点进行调节，能够更贴合人眼对亮度与对比度的联合感知特性。此外，该模型参数具有明确的物理/感知含义，便于根据直方图与场景内容进行快速调优。
