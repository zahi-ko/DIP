...
结合第\ref{cha:3}章的处理流程，完整的增强算法如下：
\begin{enumerate}
    \item \textbf{读取与预处理}：读取彩色图像并获得灰度 $I\in[0,1]$；若为彩色则保留原彩色图用于后续 HSV 映射。
    \item \textbf{背景估计}：选取滤波器与参数（推荐高斯核，窗宽 $w=71$，$\sigma\approx(w-1)/6$），计算 $I_s=I\circledast G(\sigma)$。
    \item \textbf{对比度计算}：按式\ref{actual_weber}计算 $C_W=(I-I_s)/(I_s+\epsilon)$，并提取统计量用于参数参考（参见表\ref{tab:summary_stats}）。
    \item \textbf{非线性对比度映射}：$C_W^{\text{new}}=\beta\cdot\operatorname{sgn}(C_W)\cdot |C_W|^{\alpha}$，其中 $\alpha\approx0.8$、$\beta\approx0.85$ 为经验值。
    \item \textbf{照度感知校正}：$I_s^{\text{new}}=I_s^{\gamma}$，推荐 $\gamma\approx0.4$ 以扩展暗区动态范围。
...
\end{enumerate}
复杂度方面，核心为卷积与逐点幂律/乘加运算，总体为 $O(MN)$（固定核大小）或 $O(MN\log N)$（频域实现）。参数选择可参照 $C_W$ 的直方图与统计量：当暗区占比高且噪声偏大，取较小 $\beta$ 与较大 $\gamma$；当边缘不够显著，适当提高 $\beta$ 或减小 $\alpha$ 的抑制力度。

\section{增强结果与对比分析}

在典型自然场景与低照度图像上，采用 $w=71$、$\sigma\approx(w-1)/6$、$\alpha=0.8$、$\beta=0.85$、$\gamma=0.4$ 的配置具有较好的通用性。综合表现如下：
\begin{itemize}
    ...
    \item \textbf{伪影控制}：在强边缘与高反差区域，$\beta\in(0,1]$ 有效抑制 halo 与过增强；当出现蓝通道主导的大尺度过校正（参见融合显示分析），可适当减小窗宽或引入边缘保护滤波（如引导滤波）。
    \item \textbf{色彩稳定}：HSV 的 V 替换策略避免了色相偏移，主观色彩更稳定；如需进一步提升观感，可在 $I_{\text{out}}$ 上应用受限对比度的自适应直方图均衡（CLAHE）\citing{pizer1987adaptive}，但应避免过度增强导致的噪声曝光。
\end{itemize}
与第\ref{cha:3}章的两种重建增强（伽马变换与全局直方图均衡化）相比，式\ref{eq:enh_transform} 的组合模型在保留细节、控制噪声与维持整体亮度方面更为平衡：其以局部对比度为导向、以背景照度为锚点进行调节，能够更贴合人眼对亮度与对比度的联合感知特性。此外，该模型参数具有明确的物理/感知含义，便于根据直方图与场景内容进行快速调优。
